...
Surveys are **account-wide**: they can reach engineers across all of your connected GitHub orgs, and you choose recipients **by team** (or the whole company) when sending — only people with an email on file can be sent to.

## How delivery works

1. **Map identities** — in Admin, map each engineer's GitHub handle to a work email so DevPerform knows where to send. (You can also add a missing email inline from the team roster.)
