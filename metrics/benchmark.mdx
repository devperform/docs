...
Leaving your email on a scorecard subscribes you to that repo's numbers: today's report arrives right away, and a fresh scorecard lands about monthly with the change since last time (for example, "cycle time 9.2h, was 12.0h"). One email a month per tracked repo, nothing else. Every tracking email carries a one-click unsubscribe link, and unsubscribing is permanent unless you deliberately re-submit the form.

## Privacy

The benchmark reads public PR metadata via the GitHub API. Leaving an email is optional — it subscribes you to the monthly scorecard for that repo (see above), and nothing else. No other mail, no list-sharing, unsubscribe anytime.
