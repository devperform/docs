...
- **GitLab** — delivered when DevPerform next syncs your projects (on the regular sync cadence rather than instantly), so notifications are near-real-time rather than immediate. GitLab covers **PR approved**, **PR merged**, and **CI passed / failed**; **Review requested** and **Changes requested** aren't available from GitLab's API and so aren't sent for GitLab merge requests.
